...
# ---------- 知识库分块策略（借鉴 Dify 分段设置，可在管理后台调节） ----------
KB_CHUNK_SIZE: int = int(os.getenv("KB_CHUNK_SIZE", "400"))
KB_CHUNK_OVERLAP: int = int(os.getenv("KB_CHUNK_OVERLAP", "60"))
# 预置分类（管理后台下拉），与文档自定义分类取并集
KB_PRESET_CATEGORIES: list[str] = [
    c for c in os.getenv("KB_CATEGORIES", "活动,新品,公告,常见问题,售后").split(",") if c
]
