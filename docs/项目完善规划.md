...
2. **Langfuse 接入**：全链路追踪（trace/span），无密钥时优雅降级为 no-op；环境允许则 Docker 自托管并产出真实调用链截图素材；
...
### Phase C：Langfuse 可观测性 + 文档收尾（1 天）

- [ ] `src/utils/tracing.py`：Langfuse 客户端封装（trace 包裹 handle，span 包裹各节点/LLM 调用；`LANGFUSE_*` 未配置时 no-op 并记 warning）；
- [ ] `.env.example` / `config/settings.py` 补 Langfuse 配置项；
- [ ] Docker 自托管 Langfuse（docker compose 附加服务：langfuse + postgres + redis + minio）并验证一次真实对话的调用链截图；若资源不足则降级为"文档+代码就绪，待密钥"；
- [ ] 文档同步：README（技术栈/指标/对比表）、规划文档状态栏、开发笔记（ADR-001 修订：LangGraph 编排；新增 Langfuse ADR）、简历项目描述更新；
