...
| 决策点 | 选择 | 理由 | 代价/备选 |
|--------|------|------|----------|
| 单 Agent + 路由 vs 真多 Agent | **意图路由 + 5 个专项 Agent**（共享一个 LLM 实例） | 场景边界清晰、工具集隔离、调试简单、便于逐个评测 | 若后期跨场景需要，可升级 LangGraph StateGraph |
...
>
> 针对美妆品牌客服高频重复咨询与夜间覆盖不足的痛点，设计并实现基于 RAG + 工具调用的智能客服系统，覆盖商品咨询、订单查询、退换货、护肤推荐 4 大场景。构建 40 款产品 + 160 条 FAQ 知识库，实现混合检索（Dense + BM25 + RRF 融合）+ BGE 重排序 + 引用溯源，并通过结构化分块与查询改写将检索召回率提升至 89%。基于 LangChain 实现 5 个专项 Agent 的三级意图路由（关键词→少样本分类→LLM），采用 Function Calling 与槽位状态机完成订单查询和售后工单生成；集成规则+LLM 三级情绪检测，实现"连续 2 轮不满或 1 轮愤怒自动转人工"的兜底机制。构建 100 条测试集与自动+LLM-as-Judge 双轨评测体系，量化结果：问题解决率 78%、幻觉率 ≤5%、意图路由准确率 87%、首次响应 <3s；加入 Prompt 注入防护与订单双校验防越权，通过 50 并发压测。FastAPI + WebSocket 流式 + Streamlit + Docker + Langfuse 全链路工程化部署。
